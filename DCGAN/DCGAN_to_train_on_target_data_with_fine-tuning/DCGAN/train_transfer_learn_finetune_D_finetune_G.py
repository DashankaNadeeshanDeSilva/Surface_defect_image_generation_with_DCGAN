...
def train_gan(args):
	...
	if args.netG:
		...
		#---------------------------------------------------------------
		# Transfer Learaning
		# freeze generator weights EXCEPT the last 2 layer after loading the pretrained model
		
		for name, parameter in netG.named_parameters():
			# print(name)
			if name == 'main.12.weight':
				parameter.requires_grad = True
				print('layer ', name,' NOT frozen')
			elif name == 'main.10.weight':
				parameter.requires_grad = True
				print('layer ', name,' NOT frozen')
			elif name == 'main.9.weight':
				parameter.requires_grad = True
				print('layer ', name,' NOT frozen')
			# Except the last layer all other layers are frozen
			else:
				parameter.requires_grad = False
				print('layer ', name,' is frozen')

		print('Pretrained Generator model loaded')
		#---------------------------------------------------------------
		
	# initialize network with random weights 
	else:
		print('Pre trained Generator model NOT found, initialized training from the beginning...')
	...
	if args.netD:
		...
		#---------------------------------------------------------------
		# Transfer Learaning
		# freeze discriminator weights EXCEPT the last layer after loading the pretrained model
		for name, parameter in netD.named_parameters():
			# print(name)
			if name == 'main.11.weight':
				parameter.requires_grad = True
				print('layer ', name,' NOT frozen')
			elif name == 'main.9.weight':
				parameter.requires_grad = True
				print('layer ', name,' NOT frozen')
			elif name == 'main.8.weight':
				parameter.requires_grad = True
				print('layer ', name,' NOT frozen')
			# Except the last layer all other layers are frozen
			else:
		...
		print('Pretrained Discriminator model loaded')		
		#---------------------------------------------------------------
		
	# initialize network with random weights 
	else:
		print('pre trained Discriminator model NOT found, initialized training from the beginning...')
	...
	print('Starting Tranfer-learning Training Loop....')
	# For each epoch
